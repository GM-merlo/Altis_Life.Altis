...
switch (_code) do {
        ...
        if (_shift && playerSide isEqualTo civilian && !isNull cursorObject && cursorObject isKindOf "Man" && isPlayer cursorObject && alive cursorObject && cursorObject distance player < 4 && speed cursorObject < 1) then {
            if ((animationState cursorObject) != "Incapacitated" && (currentWeapon player == primaryWeapon player || currentWeapon player == handgunWeapon player) && currentWeapon player != "" && !life_knockout && !(player getVariable ["restrained",false]) && !life_istazed && !life_isknocked) then {
				...
				if("ItemRadio" in assignedItems cursorTarget) then {
					...
					hint "Il telefono della persone e caduto a terra!";
					_defenceplace1 = "Item_ItemRadio" createVehicle (player modelToWorld[0,0,0]);
				} else { 
					hint "Non aveva nessun oggetto in tasca!"
				};
            };
        };
    };
    ...
    case 38: {
        ...
        if (_shift && playerSide in [west,independent]) then {
            if (!(isNull objectParent player) && (typeOf vehicle player) in ["C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F","C_Hatchback_01_sport_F","B_Heli_Light_01_F","C_Hatchback_01_F","B_Heli_Transport_01_F"]) then {
	...
	case 24:
    {
		if(_shift) then 
		{
			...
			{
				case 0: {hintSilent "Tappi 90%"; 1 fadeSound 0.1; player setVariable ["Earplugs", 10]; };
				case 10: {hintSilent "Tappi 60%"; 1 fadeSound 0.4; player setVariable ["Earplugs", 40]; };
				case 40: {hintSilent "Tappi 30%"; 1 fadeSound 0.7; player setVariable ["Earplugs", 70]; };
				case 70: {hintSilent "Tappi Rimossi"; 1 fadeSound 1; player setVariable ["Earplugs", 0]; };
			};
		};
