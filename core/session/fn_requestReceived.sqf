...
if(profileName != _this select 1) exitWith {
	hint format["VECCHIO NICK: %1",_this select 1];
	[format["Per la tua chiave di gioco c'è un'altro nome, metti quello vecchio!<br/><br/>Vecchio nome <t color='#b20303'>%1</t><br/><br/>Se hai qualche problema a cambiare nome - vai sul ts <t color='#665bff'>ts.lioncommunity.it</t>",_this select 1],
	"cambio nickname registrato",
	"adotta"
